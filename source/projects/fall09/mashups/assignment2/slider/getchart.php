<?php
    ...
    // Takes an date integer as a parameter and returns either an ordered list of the top ten results, or an error message
    function getChartForValidDate($userdate_as_time) {
        ...
        if (count($merged_array) > 0) {
            ...
            for ($i = 0; $i < 10; $i++) {
                ...
                $html = $html .$merged_array[$i]['artist'] . ' - ' . $merged_array[$i]['song'];
            ...
            return "No results found for this date, even though it is within the permitted range.";
        }
    }
    ...
    if (isset($_GET["date"])) {
      ...
      if ($validation_results[0]) {
          echo getChartForValidDate($validation_results[1]);
      }
    }
?>